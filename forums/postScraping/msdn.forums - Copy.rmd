...
The correlation between replyCount and votes was greater than 0.5 and not considered here. So the focus was then more towards viewCount and votes. Among the three, the number of views and the number of replies were least coreleated, while between the number of votes and the number replies the correlation pronounced most.
...
## Considering Linear Regression Between votes and viewCount
...
### For votes and viewCount, examining 
$$\hat{votes} = \beta~0 + \beta~1 * viewCount$$

Here's how this model looked:

```{r echo=FALSE, message=FALSE, warning=FALSE}

FITvvc <- lm( v ~ log(vc), data=vcrcv)

summary(FITvvc)
cat('Confidence Interval ',confint(FITvvc))
summary(FITvvc)$coefficients

print(c('beta0',summary(FITvvc)$coefficients[1,1]))
print(c('beta1',summary(FITvvc)$coefficients[2,1]))
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Normality
qqnorm(log(FITvvc$residuals))
qqline(log(FITvvc$residuals))
plot(FITvvc)
#influencePlot(FITvvc)
```
...
Apply $\beta~0$ and $\beta~1$ into the model,
$$\hat{votes} = -5.363812 + 1.195719 * viewCount$$
Notice throughout the analysis, all numbers are logarithmic. Therefore

- when $$\hat{votes}= 1$$, 
$$viewCount = exp(1+5.363812/1.195719)$$

```{r echo=FALSE, message=FALSE, warning=FALSE}
view_count = exp(
  (1+abs(summary(FITvvc)$coefficients[1,1]))/(summary(FITvvc)$coefficients[2,1])
)
view_count

```

*This simple linear model therefore predicts an estimated and average 204 views for receiving a vote.*

```{r echo=FALSE, message=FALSE, warning=FALSE}
vcrcv = subset(forums, select=(c('viewCount','replyCount','votes', 'threadState')))
vc <- forums$viewCount[0:200]
rc <- forums$replyCount[0:200]
v  <- forums$votes[0:200]
#lm( v ~ log(vc), data=vcrcv)
fit2 = lm( v ~ vc + rc, data = vcrcv, na.action=na.pass)
summary(fit2)
cat('Confidence Interval ',confint(fit2))
summary(fit2)$coefficients


isError <- is.na(v) || is.nan(v) || is.infinity(v)
...
fit1 = lm( vcrcv$votes ~ vcrcv$viewCount , data = vcrcv)
	
pfit1  = RegressionPlots(fit1)

pfit1
```

## Experimenting and still under investigation
...
fit2 = lm( vcrcv$votes ~ vcrcv$viewCount + vcrcv$replyCount, data = vcrcv)
	
pfit2  = RegressionPlots(fit2)
